...
const generateMCQ = (meaning: {
    ...
    const ansExceptCorrectOne = meaning.filter((i) => i.Text !== correctAns);

    const incorrectAns: string[] = _.sampleSize(ansExceptCorrectOne, 3).map((i) => i.Text)
...
export const translateWords = async (params: LangType) => {
    try {
        ...
        const rapidKey = import.meta.env.VITE_TRANSLATE_API;
        ...
        const arr: WordType[] = receivedData.map((i, idx) => {
...
export const countMatchingElemet = (arr1: string[], arr2: string[]): number => {
